...
class Dataset(object):
    ...
    def __init__(self, file_path, token_dicts, data_field_list, label_field,  \
        ...
        self.variable_shape_index_list = [ self.get_variable_shape_index(x.shape) for x in data_field_list ]
        ...
        if not self.is_batch_padding:
            for i in range(len(self.variable_shape_index_list)):
                if self.variable_shape_index_list[i] is None:
                    raise Exception("shape index", i, self.data_field_list[i].shape,"not valid")
        ...
        for filename in self.filenames:
            ...
            for line in self.read_input_lines(filename):
                ...
                if self.weight_fn is not None:
                    ...
                    #yield additive_info, tuple(features), label, self.weight_fn(line)
    ...
    def _get_shapes(self, is_training):
        #shapes = tuple(
        #    [None for field in self.data_field_list if field.processor is not None])
        #if self.is_with_label:
        ...
        shapes = tuple(shapes)
        #shapes = tuple(
        #    [field.shape for field in self.data_field_list if field.processor is not None])
        if is_training:
            ...
            if self.is_with_label:
                shapes = tuple([(len(self.additive_schema)), shapes, self.label_field.shape])
            else:
                shapes = tuple([len(self.additive_schema), shapes])
        return shapes

    def _get_types(self, is_training):
        types = []
        for field in self.data_field_list:
            if field.processor is None:
                continue
        ...
        types = tuple(types)
        #types = tuple(
        #    [field.type for field in self.data_field_list if field.processor is not None])
        if is_training:
            if self.weight_fn is None:
