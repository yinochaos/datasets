...
class RawDataset(Dataset):
    ...
    def _parse_fn(self, line):
        ...
        features = [data_processor_dicts[self.data_field_list[index].processor](data_fields[index], self.data_field_list[index], self.token_dicts) if self.data_field_list[index].processor is not None else None for index in range(0, data_num_fields)]
        if self.is_batch_padding:
            ...
            for i in range(0, data_num_fields):
                ...
                if len(self.variable_shape_index_list[i]) == 1:
                    return_features.append(features[i].shape[self.variable_shape_index_list[i][0]])
                elif len(self.variable_shape_index_list[i]) > 1:
                    return_features.append(np.asarray([features[i].shape[k] for k in self.variable_shape_index_list[i]], dtype='int32'))
